...
counts <- read_tsv(system.file(
  "extdata",
  "LIHC_HTseqCounts.txt.gz",
  package = "MOSuite"
)) |>
  rename(gene_id = Gene)
metadat <- read_tsv(system.file(
  "extdata",
  "LIHC_PatientData.txt.gz",
  package = "MOSuite"
)) |>
  select(-sample_id) |>
...
```{r subset}
subset_mem <- lapply(
  c(10, 50, 100, 200, nrow(moo@sample_meta)),
  function(nsamples) {
    samples_subset <- moo@sample_meta |> slice_sample(n = nsamples)
    counts_subset <- moo@counts$raw |>
      dplyr::select(
        gene_id,
        tidyselect::all_of(samples_subset |> dplyr::pull(sample_id))
      )
...
subset_mem |>
  ...
  scale_y_continuous(
    labels = scales::label_bytes(units = "GB", accuracy = 0.1)
  ) +
...
dat_comp <- bind_rows(
  subset_mem |>
    mutate(dataset_type = "bulk (S7)", object_size = as.double(object_size)),
  sc_dat
)
dat_comp |>
  ...
  labs(
    title = "Memory usage of bulk & single-cell RNA-seq objects",
    x = "Number of samples",
    y = "Memory usage"
  ) +
  theme_bw()
